...
const downloadMedia = async (msg: proto.IWebMessageInfo, isImported: Date = null) => {
  ...
  //console.log("filename: ", msg.message?.documentMessage);
...
const verifyQueue = async (
  ...
  if (queues.length === 1 && !chatbot) {
    ...
    if (enableQueuePosition) {
      ...
      const tempSettingMessage = await FindCompanySettingOneService({companyId, column: "queuePositionMessage"});
      const settingMessage = tempSettingMessage[0];
      const qtd = count.count === 0 ? 1 : count.count
      let msgFila = '';
      if((settingMessage?.queuePositionMessage?.trim() !== "") && settingMessage?.queuePositionMessage){
        msgFila = `${settingMessage?.queuePositionMessage} *${qtd}*`;
      }else{
        msgFila = `*Assistente Virtual:*\n{{ms}} *{{name}}*, sua posição na fila de atendimento é: *${qtd}*`;
      }
