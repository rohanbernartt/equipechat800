...
const wbotMonitor = async (
  ...
  try {
    wbot.ws.on("CB:call", async (node: BinaryNode) => {
      ...
      if (content.tag === "terminate" && !node.attrs.from.includes('@call')) {
        ...
        if (settings.acceptCallWhatsapp === "enabled") {
          const tempMessageSetting = await FindCompanySettingOneService({companyId, column:"refuseTelephoneCallMessage"});
          const messageSetting = tempMessageSetting[0];

          if((messageSetting?.refuseTelephoneCallMessage?.trim() !== '') && messageSetting?.refuseTelephoneCallMessage){
            await wbot.sendMessage(node.attrs.from, {
              text: messageSetting?.refuseTelephoneCallMessage,
            });
          }else{
            await wbot.sendMessage(node.attrs.from, {
              text:
    ...
    //   const newContact = createOrUpdateBaileysService({
