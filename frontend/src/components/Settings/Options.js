...
export default function Options(props) {
  ...
  const [greetingMessageTicket, setGreetingMessageTicket] = useState("");
  const [refuseTelephoneCallMessage, setRefuseTelephoneCallMessage] = useState("");
  const [queuePositionMessage, setQueuePositionMessage] = useState("");
  ...
  useEffect(() => {
    for (const [key, value] of Object.entries(settings)) {
      ...
      if (key === "transferMessage") setTransferMessage(value ? value : "")
      if (key === "greetingMessageTicket") setGreetingMessageTicket(value ? value : "")
      if (key === "refuseTelephoneCallMessage") setRefuseTelephoneCallMessage(value ? value : "")
      if (key === "queuePositionMessage") setQueuePositionMessage(value ? value : "")
    }    
  }, [settings]);
  ...
  const handleChangeTransferMessage = async (value)=> {
  ...
  const handleChangeGreetingMessageTicket = async (value)=> {
    setGreetingMessageTicket(value);
    await update({
  ...
  const handleChangeRefuseTelephoneCallMessage = async (value)=> {
    setRefuseTelephoneCallMessage(value);
    await update({
      column:"refuseTelephoneCallMessage",
      data:value
    });
  }

  const handleChangeQueuePositionMessage = async (value)=> {
    setQueuePositionMessage(value);
    await update({
      column:"queuePositionMessage",
      data:value
    });
  }
    ...
    <>
      ...
      <div>
        <Grid spacing={2} container>
          <Grid Grid xs={12} sm={12} md={6} item>
            <TextField
              as={TextField}
              label={i18n.t("settings.settings.options.transferMessage")}
              multiline
              rows={4}
              fullWidth
              ...
              onChange={(e)=>handleChangeTransferMessage(e.target.value)}
            />
          </Grid>
          <Grid Grid xs={12} sm={12} md={6} item>
            <TextField
              as={TextField}
              label={i18n.t("settings.settings.options.greetingMessageTicket")}
              multiline
              rows={4}
              fullWidth
              name="greetingMessageTicket"
              variant="outlined"
              margin="dense"
              value={greetingMessageTicket}
              onChange={(e)=>handleChangeGreetingMessageTicket(e.target.value)}
            />
          </Grid>
          <Grid Grid xs={12} sm={12} md={6} item>
            <TextField
              as={TextField}
              label={i18n.t("settings.settings.options.refuseTelephoneCallMessage")}
              multiline
              rows={4}
              fullWidth
              name="refuseTelephoneCallMessage"
              variant="outlined"
              margin="dense"
              value={refuseTelephoneCallMessage}
              onChange={(e)=>handleChangeRefuseTelephoneCallMessage(e.target.value)}
            />
          </Grid>
          <Grid Grid xs={12} sm={12} md={6} item>
            <TextField
              as={TextField}
              label={i18n.t("settings.settings.options.queuePositionMessage")}
              multiline
              rows={4}
              fullWidth
              ...
              variant="outlined"
              margin="dense"
              value={queuePositionMessage}
              onChange={(e)=>handleChangeQueuePositionMessage(e.target.value)}
            />
          </Grid>
        </Grid>
